...
public interface HotelRepository extends BaseRepository<Hotel, Long>{
    ...
    @Query(value = "SELECT * FROM hotel h WHERE h.name LIKE %:name%", nativeQuery = true)
    List<Hotel> findHotelByKeyword(@Param("name") String name);
    ...
    @Query(value = "SELECT * FROM hotel h WHERE " +
            ...
            "AND (:priceStart IS NULL OR h.price * (100 - h.sale) / 100 >= :priceStart) " +
            "AND (:priceEnd IS NULL OR h.price * (100 - h.sale) / 100 <= :priceEnd) " +
